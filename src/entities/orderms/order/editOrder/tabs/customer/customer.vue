<template>
    <div>
                ...
                <form name="editForm" role="form" novalidate v-on:submit.prevent="save()">
                    <div>
                        <div class="row">
                            <div class="col-md-6 form-group">
                                ...
                                <select class="form-control" v-model="orderCopy.languageCode" @change="changeLangauge">
                                    <option v-for="(item, index) in $store.state.languages" :value="index" :key="index">{{item['name']}}</option>
                                </select>
                            </div>
                            <div class="col-md-6 form-group">
                                <label class="form-control-label">{{$t('labels.startDate')}}</label>
                                <div class="date-input">
                                    <flat-pickr :config="startDateConfig" v-model="startDate" class="single-daterange form-control"></flat-pickr>
                                </div>
                            </div>
                        </div>
                        <div class="row" v-if="orderCopy.orderCustomer && orderCopy.orderCustomer.email">
                            ...
                            <div class="col-md-6 form-group">
                                <br/>
                                <button type="button" data-toggle="modal" data-target="#sendMail" class="btn btn-primary">{{$t('labels.sendEmail')}}</button>
                            </div>
                        </div>
                        <div class="row" v-if="orderCopy.orderCustomer && orderCopy.orderCustomer.fullName">
                        ...
                        <div class="row" v-if="orderCopy.customerBillingAddress && orderCopy.customerBillingAddress.id">
                            <div class="col-md-3 mt-3">
                                <label class="form-control-label">{{$t('labels.BillingAddress')}}</label>
                                <input type="text" class="form-control" :placeholder="$t('labels.street')" v-model="orderCopy.customerBillingAddress.street"/>
                            </div>
                            <div class="col-md-3 mt-3">
                                <br/>
                                <input type="text" class="form-control" :placeholder="$t('labels.houseNumber')" v-model="orderCopy.customerBillingAddress.houseNumber"/>
                            </div>
                            <div class="col-md-3 mt-3">
                                <br/>
                                <input type="text" class="form-control" :placeholder="$t('labels.city')" v-model="orderCopy.customerBillingAddress.city"/>
                            </div>
                            <div class="col-md-3 mt-3">
                                <br/>
                                <input type="text" class="form-control" :placeholder="$t('labels.postalCode')" v-model="orderCopy.customerBillingAddress.postalCode"/>
                            </div>
                            <div class="col-md-3 mt-3">
      ...
      <div class="modal" data-backdrop="static" data-keyboard="false" id="sendMail" tabindex="-1" role="dialog" ref="sendMail">
            ...
            <div class="modal-body text-left">
              <div class="mt-4">
                ...
                <div class="form-group">
                  ...
                  <MultiLangHtmlEditorComponent :content.sync="invoiceEmailContent" @contentChanged="updateInvoiceEmailContent"/>
                </div>
              </div>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-secondary" data-dismiss="modal">{{$t('buttons.close')}}</button>
              <button type="button" class="btn btn-primary" @click="sendMail" data-dismiss="modal">{{$t('buttons.save')}}</button>
            </div>
          </div>
        </div>
      </div>
