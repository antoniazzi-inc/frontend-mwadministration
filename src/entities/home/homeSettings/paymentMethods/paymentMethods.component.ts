...
import {IPaymentMethod, PaymentMethod, PaymentMethodType, PaymentMethodAvailability} from "@/shared/models/payment-method.model";
      ...
      if (success) {
        ...
        if (this.paymentMethod.id) {
          this.deliveryMethodService.put(this.paymentMethod).then((resp: AxiosResponse) => {
            if (resp) {
              // @ts-ignore
              this.$vueOnToast.pop('success', '', this.$t('toastMessages.paymentMethodUpdated'))
            } else {
              //@ts-ignore
          ...
          this.deliveryMethodService.post(this.paymentMethod).then((resp: AxiosResponse) => {
            if (resp) {
              // @ts-ignore
              this.$vueOnToast.pop('success', '', this.$t('toastMessages.paymentMethodCreated'))
            } else {
              //@ts-ignore
              this.$vueOnToast.pop('error', this.$t('toastMessages.error'), this.$t('toastMessages.paymentMethodError'))
            }
