...
export default class RelationImportComponent extends mixins(CommonHelpers, Vue) {
  ...
  public validateStep() {
    ...
    return new Promise(resolve => {
      ...
      if (self.step === 1 && self.file) {
        ...
        self.step3();
        resolve(true)
      } else resolve(this.file ? true : false)
    })
  }
    ...
    if (me.fileType === 'csv') {
      ...
      reader.onload = function (e: any) {
        ...
        data.forEach(function (value: any, index: any) {
          ...
          let parsedData = me.parseVcf(value);
          if (parsedData.fn !== undefined) {
            let myObj: any = {};
            let fullName = parsedData.fn.split(' ');
            let firstName = "";
            let lastName = "";
          ...
          if (parsedData.adr !== undefined) {
            let myObj: any = {};
            let adressVal = parsedData.adr[0];
            let street = adressVal.value[0].concat(adressVal.value[1], adressVal.value[2]);
      ...
      reader.onload = function (e: any) {
        ...
        if (me.fileType === 'xlsx' || me.fileType === 'xlsm' || me.fileType === 'xlsb') {
          ...
          let cfb = XLSX.CFB.read(data, {
            type: 'binary'
          });
          let workbook = XLSX.parse_xlscfb(cfb);
          let sheet_name_list = workbook.SheetNames;
  ...
  filename() {
    if (this.file) {
      let fileSize = this.file.size / 1048576;
        ...
        let results = Papa.parse(file,
          {
            ...
            complete: function (results: any) {
              ...
              let foundFields = self.rows[0].filter(function (obj: any, objindex: number) {
  ...
  public step3() {
    ...
    if (self.datafordb.length < 1) {
      ...
      let emailFieldIndex = self.mappings.findIndex(x => x.dbfield == 'email');
      ...
      self.rows.forEach(function (rowValue, rowIndex) {
        if (!self.vcfFile) {
            ...
            if (rowValue[self.emailFieldIndex] != undefined && rowValue[self.emailFieldIndex] != '') {
              ...
              if (foundDuplicate != undefined) {
                ...
                rowValue.forEach(function (cellValue: any, cellIndex: any) {
                  counter += 1;
                  let columnName = self.mappings[cellIndex].dbfield;
                  ...
                  if (columnName != 'dnassign') {
                    ...
                    if (self.relationsToImport < 3) {
                      let foundDbField = self.dbfields.filter(x => x.value === columnName);
          ...
          let foundKey = rowValue.filter((e: any) => e.hasOwnProperty('email'));
          ...
          if (foundKey.length > 0 && foundKey[0].email !== '') {
            ...
            if (foundDuplicate != undefined) {
              ...
              rowValue.forEach(function (cellValue: any, cellIndex: any) {
                ...
                let key = Object.keys(cellValue);
                if (key[0] == 'email') {
                  // make email lowercase
                  cellValue.email = cellValue.email.toLowerCase();
                }
                // console.log('dup', foundDuplicate);
                let foundDbField = self.dbfields.filter(x => x.value === key[0]);
