  ...
  <div class="example-full">
      ...
      <div class="table-responsive" style="min-height: 300px">
        <table class="table table-hover">
          ...
          <tr v-for="(file, index) in files" :key="file.id" :class="{'text-danger': maxFileSizeExceedError}">
            <template v-if="allFiles.findIndex(e=> e.id===file.id) > -1">
              <td>{{ index }}</td>
              <td>
              ...
              <td>{{ file.file.size | formatSize }}</td>
                ...
                <div class="btn-group">
                  ...
                  <div class="dropdown-menu">
                    <a
                      ...
                      data-toggle="modal" data-target="#modal-edit-file" href="#"
                      @click.prevent="file.active || file.success || file.error === 'compressing' ? false :  onEditFileShow(file)">{{ $t('buttons.edit') }}</a>
                    <a :class="{'dropdown-item': true, disabled: !file.active}" href="#"
                       @click.prevent="file.active ? $refs.upload.update(file, {error: 'cancel'}) : false">{{ $t('buttons.cancel') }}</a>
   ...
   -->
                    ...
                    <a class="dropdown-item" href="#"
                       @click.prevent="$refs.upload.remove(file)">{{ $t('buttons.remove') }}</a>
                  </div>
                </div>
              </td>
      ...
      <div class="example-foorer">
        <div class="btn-group">
          ...
          <div :class="{'dropdown-menu': directory ? true : false}">
            ...
            <a class="dropdown-item" href="#" v-if="directory"
               @click.prevent="addData.show = true">{{ $t('labels.addData') }}</a>
          </div>
        </div>
        <button type="button" class="btn btn-success" v-if="directUpload && (!$refs.upload || !$refs.upload.active)"
                @click.prevent="$refs.upload.active = true">
          <i class="fa fa-arrow-up" aria-hidden="true"></i>
          {{ $t('labels.startUpload') }}
        </button>
        <button type="button" class="btn btn-danger" v-else-if="directUpload"
    ...
    <div data-backdrop="static" data-keyboard="false" :class="{modal: true}" id="modal-add-data" tabindex="-1"
         role="dialog">
          ...
          <form @submit.prevent="onAddData">
            <div class="modal-body">
              <div class="form-group">
                ...
                <input type="text" class="form-control" required id="name1" placeholder="Please enter a file name"
                       v-model="addData.name">
                <small class="form-text text-muted">Such as <code>filename.txt</code></small>
              </div>
              <div class="form-group">
                <label for="type">Type:</label>
                <input type="text" class="form-control" required id="type" placeholder="Please enter the MIME type"
    ...
    <div data-backdrop="static" ref="editImageModal" data-keyboard="false" :class="{modal: true}" id="modal-edit-file"
          ...
          <form @submit.prevent="onEditorFile">
              ...
              <div class="form-group"
                ...
                <div class="edit-image-tool">
                  <div class="btn-group" role="group">
                    <button type="button" class="btn btn-primary" @click="rotateLeft" title="cropper.rotate(-90)"><i
                      class="fa fa-undo" aria-hidden="true"></i></button>
                    <button type="button" class="btn btn-primary" @click="rotateRight" title="cropper.rotate(90)"><i
                      class="fa fa-repeat" aria-hidden="true"></i></button>
                  </div>
                  <div class="btn-group" role="group">
                    <button type="button" class="btn btn-primary" @click="crop" title="cropper.crop()"><i
                      class="fa fa-check" aria-hidden="true"></i></button>
                    <button type="button" class="btn btn-primary" @click="clear" title="cropper.clear()"><i
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-secondary" data-dismiss="modal"
                      @click.prevent="editFile.show = false">{{ $t('buttons.close') }}
              </button>
