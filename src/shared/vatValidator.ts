...
export function checkVATNumber (toCheck) {
  ...
  //    1.  Change the country code in the defCCode letiable below to "DE".
...
// @ts-ignore
  ...
  for (let i = 0; i < vatexp.length; i++) {
    ...
    if (vatexp[i].test(VATNumber)) {
      ...
      let cNumber = RegExp.$2                           // Isolate the number
...
function BGVATCheckDigit (vatnumber) {
  ...
  if (vatnumber.length === 9) {
    ...
    let temp = 0
    // @ts-ignore
    for (let i = 0; i < 8; i++) temp += Number(vatnumber.charAt(i)) * (i + 1)
    ...
    // @ts-ignore
    temp = 0
    for (let i = 0; i < 8; i++) temp += Number(vatnumber.charAt(i)) * (i + 3)
  ...
  if ((/^\d\d[0-5]\d[0-3]\d\d{4}$/).test(vatnumber)) {
    ...
    // Check month
    let month = Number(vatnumber.slice(2, 4))
...
function CHEVATCheckDigit (vatnumber) {
  ...
  // Extract the next digit and multiply by the counter.
  let multipliers = [5, 4, 3, 2, 7, 6, 5, 4]
  let total = 0
...
function CZVATCheckDigit (vatnumber) {
  ...
  // Individuals type 2 (Special Cases) - 9 digits including check digit
    ...
    // Extract the next digit and multiply by the counter.
    for (let i = 0; i < 7; i++) total += Number(vatnumber.charAt(i + 1)) * multipliers[i]

    // Establish check digit pointer into lookup table
    if (total % 11 === 0) {
      let a = total + 11
    } else {
      let a = Math.ceil(total / 11) * 11
    }
    let pointer = a - total

    // Convert calculated check digit according to a lookup table;
    let lookup = [8, 7, 6, 5, 4, 3, 2, 1, 0, 9, 8]
  ...
  // Individuals type 3 - 10 digits
    ...
    let temp = Number(vatnumber.slice(0, 2)) + Number(vatnumber.slice(2, 4)) + Number(vatnumber.slice(4, 6)) + Number(vatnumber.slice(6, 8)) + Number(vatnumber.slice(8))
...
function GBVATCheckDigit (vatnumber) {
  ...
  let no = Number(vatnumber.slice(0, 7))
...
function LTVATCheckDigit (vatnumber) {
  ...
  if (vatnumber.length === 9) {
    ...
    let total = 0
    for (let i = 0; i < 8; i++) total += Number(vatnumber.charAt(i)) * (i + 1)

    // Can have a double check digit calculation!
    if (total % 11 === 10) {
      let multipliers = [3, 4, 5, 6, 7, 8, 9, 1]
      total = 0
  ...
  // 12 character VAT numbers are for temporarily registered taxpayers
    ...
    for (let i = 0; i < 11; i++) total += Number(vatnumber.charAt(i)) * multipliers[i]

    // Can have a double check digit calculation!
    if (total % 11 === 10) {
      let multipliers = [3, 4, 5, 6, 7, 8, 9, 1, 2, 3, 4]
      total = 0
...
function NLVATCheckDigit (vatnumber) {
  ...
  function soleproprietorNo (vatnumber) {
    ...
    let numericvat = convert_string_tonumeric(vatnumber)
...
function RUVATCheckDigit (vatnumber) {
  ...
  if (vatnumber.length === 10) {
    let total = 0
    let multipliers = [2, 4, 10, 3, 5, 9, 4, 6, 8, 0]
    for (let i = 0; i < 10; i++) {
      total += Number(vatnumber.charAt(i)) * multipliers[i]
    }
    total = total % 11
    ...
    let total1 = 0
    let multipliers1 = [7, 2, 4, 10, 3, 5, 9, 4, 6, 8, 0]
    let total2 = 0
    let multipliers2 = [3, 7, 2, 4, 10, 3, 5, 9, 4, 6, 8, 0]

    for (let i = 0; i < 11; i++) total1 += Number(vatnumber.charAt(i)) * multipliers1[i]
    total1 = total1 % 11
    if (total1 > 9) {
      total1 = total1 % 10
    }

    for (let i = 0; i < 11; i++) total2 += Number(vatnumber.charAt(i)) * multipliers2[i]
    total2 = total2 % 11
    if (total2 > 9) {
      total2 = total2 % 10
    }
...
function SEVATCheckDigit (vatnumber) {
  ...
  let S = 0
  for (let i = 1; i < 9; i = i + 2) S += Number(vatnumber.charAt(i))
